  ...
  test.describe("Desktop Navigation", () => {
    ...
    test("solutions dropdown should open on click", async ({ page }) => {
      ...
      const solutionsToggle = page
        .locator("header .dropdown-toggle")
      ...
      if ((await solutionsToggle.count()) > 0) {
        ...
        const hasShowClass = await dropdownMenu
          .getAttribute("class")
          .then((c) => c?.includes("show"))
          .catch(() => false);
    ...
    test("header should change class on scroll", async ({ page }) => {
      ...
      const classesChanged =
        initialClasses !== scrolledClasses ||
  ...
  test.describe("Active Navigation State", () => {
    ...
    test("header link should be active on matching page", async ({ page }) => {
      ...
      const activeLink = page.locator(
        'header .header-link.active[href="/unified-commerce"]',
      );
    ...
    test("footer nav link should be active on matching page", async ({
      ...
      const footerActiveLink = page.locator(
        'footer .flex.flex-col a.active[href="/unified-commerce"]',
      );
    ...
    test("no header links should be active on non-matching page", async ({
      ...
      const activeLink = page.locator(
        'header .header-link.active[href="/about-us"]',
      );
      await expect(activeLink).toHaveCount(1);
    });
  });
