...
- **Redirects**: `vercel.json` `redirects` are simulated by `devRoutingPlugin` in `vite.config.js` for both `npm run dev` and `npm run preview`, so `tests/e2e/redirects.spec.js` can request every source and assert the status Vercel sends (308 for permanent, 307 otherwise, query string kept) and its destination for real
...
Writer marks Notion page "Ready to Publish"
        │
        ▼
GitHub Action (manually triggered)
...
| `scripts/log-notion-history.js`     | Logs each sync run (status, article count/titles, PR link) to a "Sync History" Notion database, see [`docs/notion-blog-developer-guide.md`](notion-blog-developer-guide.md#sync-history-logging) |
