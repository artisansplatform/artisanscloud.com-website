...
| Conventions      | `tests/conventions.test.js`     | inline executable scripts, duplicate Swiper selectors, broken/ shadowing redirects, redirect stubs left in the sitemap, em dashes (literal, HTML entity, or spaced en dash) in tracked and untracked files, shell-string child processes (tracked and untracked), POSIX-only npm scripts, a rotted Claude Code Stop hook (missing script, missing suite, a `test:fast` script that has drifted from the hook's suite list, or a tree digest that cannot see an untracked file) |
