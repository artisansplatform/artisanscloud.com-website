...
| Conventions      | `tests/conventions.test.js`     | inline executable scripts, duplicate Swiper selectors, broken/ shadowing redirects, redirect stubs left in the sitemap, em dashes (literal, HTML entity, or spaced en dash) in tracked and untracked files, shell-string child processes |
...
Never spawn a child process with a command string (`execSync(cmd)`, `spawnSync(cmd)`, `shell: true`, and similar). A command string is re-parsed by a shell, and on Windows that shell is `cmd.exe` with different quoting rules than POSIX, so a path with a space, glob character, or quote can break or, worse, be reinterpreted. Use the array form instead: `execFileSync(cmd, [args], { cwd, encoding })`. `tests/conventions.test.js` enforces this across `scripts/`, `tests/`, `api/`, and the root config files.
