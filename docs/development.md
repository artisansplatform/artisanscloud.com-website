...
| Brand colors     | `tests/brand-colors.test.js`    | theme tokens that no longer match the brand hexes in the master logo, and any reappearance of a superseded hex (tracked and untracked files)                                                                                                                                                                                                                                                                                                                                                              |
...
`tests/brand-colors.test.js` parses the `fill` / `stroke` / `stop-color` values out of the logo, requires all three brand colors to still be present there, and requires the matching theme tokens to equal them exactly. A second check bans the three superseded values (listed as `superseded` in the test's `BRAND` table) from every tracked and untracked source file, so a copy-pasted old hex cannot quietly reintroduce the drift. `--color-primary-light` and `--color-sky-2` are hover and secondary shades with no brand counterpart and are not checked.

Reach for the token utility (`text-primary`, `bg-primary`, `text-sky`, `bg-pink`, ...) rather than an arbitrary value like `text-[#8d67f5]`. Inline SVG markup that cannot use a utility should carry the brand hex from the logo. If the brand itself ever changes, update the logo, the `@theme` tokens, and the `BRAND` table in the test together.
