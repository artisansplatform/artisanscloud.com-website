...
- Breadcrumb names never carry the brand: `pageName()` in `scripts/lib/page-meta.js` strips "Artisans Cloud" (and anything after it) from the `<title>` wherever it sits, so a new title shape that leaks it fails here.
...
| Formatting       | `tests/format.test.js`          | files prettier would rewrite, grandfathered entries that have become clean, and `.prettierignore` drifting from `GRANDFATHERED`                                                                                                                                                                                                                                                                                                                                                                           |
