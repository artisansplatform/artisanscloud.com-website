...
- **Per-page `<head>` is hand-maintained and duplicated** - title, description, canonical, OG/Twitter tags live in each page's `<head>`, not in a partial. A change to head conventions must be applied to every page (and to the `scripts/generate-team-cards.js` template for `team/*.html`). Until the planned page-template refactor lands, sweep with `sed`/`awk` and re-run `npm run test:seo`.
- **URL Redirects on File Rename** - Every time an AI or user renames a file (changing an existing URL), a corresponding redirect rule from the old URL to the new URL MUST be added to the `redirects` list in `vercel.json`.
