...
export function initMultiSelect() {
  ...
  selectElements.forEach((select) => {
    ...
    dropdown.className =
      "absolute inset-x-0 mt-1 bg-white border border-gray-200 rounded-[12px] max-h-[220px] overflow-y-auto overscroll-contain z-50 hidden";
    ...
    [...select.options].forEach((opt) => {
      ...
      item.className =
        "flex items-center gap-2.5 px-[14px] py-[10px] cursor-pointer text-[14px] hover:bg-[#f3f6fb]";
      item.innerHTML = `
        <span
    ...
    function renderSelected() {
      ...
      visible.forEach(({ opt, item }) => {
        ...
        pill.className =
          "inline-flex items-center gap-1.5 bg-[#E5F1F7] px-2 pe-1 rounded-full text-[14px] text-[#222222] whitespace-nowrap flex-shrink-0";
      ...
      if (remaining > 0) {
        ...
        more.className =
          "inline-flex bg-[#E5F1F7] text-[#222222] rounded-full px-[5px] py-0.5 text-[14px] flex-shrink-0";
