...
async function generateVCardString(data) {
  ...
  if (data.social?.linkedin)
    lines.push(
      foldVCardLine(`X-SOCIALPROFILE;TYPE=linkedin:${data.social.linkedin}`),
    );
  if (data.social?.twitter)
    lines.push(
      foldVCardLine(`X-SOCIALPROFILE;TYPE=twitter:${data.social.twitter}`),
    );
  if (data.social?.github)
    lines.push(
      foldVCardLine(`X-SOCIALPROFILE;TYPE=github:${data.social.github}`),
    );
  if (data.location)
    lines.push(foldVCardLine(`ADR;TYPE=WORK:;;${data.location};;;;`));
  ...
  if (data.photo) {
    ...
    if (photo) {
      lines.push(
        foldVCardLine(`PHOTO;ENCODING=b;TYPE=${photo.typeStr}:${photo.base64}`),
      );
    } else {
      // Fallback to URI reference if fetch fails
      lines.push(
        foldVCardLine(
          `PHOTO;VALUE=URI:https://www.artisanscloud.com${data.photo}`,
        ),
      );
    }
  }
  ...
  if (emailBtn) {
    emailBtn.addEventListener("click", (e) => {
      ...
      const subject = encodeURIComponent(
        `${data.name} - Digital Business Card`,
      );
      const body = encodeURIComponent(
        `Connect with ${data.name} at ${data.company}:\n${cardUrl}`,
      );
  ...
  if (whatsappBtn) {
    whatsappBtn.addEventListener("click", (e) => {
      ...
      const text = encodeURIComponent(
        `Connect with ${data.name} at ${data.company}: ${cardUrl}`,
      );
