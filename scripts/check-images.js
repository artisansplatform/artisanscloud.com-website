...
if (oversized.length === 0) {
  if (paths.length > 0) {
    console.log(`Image check passed (${paths.length} file${paths.length === 1 ? '' : 's'} under size limits).`);
  }
  process.exit(0);
}

console.error('');
console.error(`Found ${oversized.length} oversized image${oversized.length === 1 ? '' : 's'}:`);
console.error('');
...
if (svgOversized.length > 0) {
  console.error('SVGs are not auto-optimized. Run them through https://jakearchibald.github.io/svgomg/');
  console.error('or strip embedded raster data manually.');
  console.error('');
}
console.error('To bypass this check for an exceptional case, commit with --no-verify.');
console.error('');
